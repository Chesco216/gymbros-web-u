...
export const GymCard = ({ gym }) => {
	...
	return (
		<li className={`fade-in-section ${isVisible ? 'is-visible' : ''} flex bg-white rounded-xl shadow-lg hover:bg-gray-50 w-full cursor-pointer transition-all ease-in-out hover:scale-[1.020] h-[440px] lg:h-auto pb-10 border border-gray-300 duration-200`} ref={sectionRef}>
			<NavLink to={`/gyms/${gym.uid}`} className="flex flex-col w-full items-start gap-4 h-[440px]">
				<img src={gym.main_image} alt="" className="w-full h-32 xl:h-64 object-cover rounded-t-xl" />
				<div className="flex flex-col w-full h-64 xl:h-32 gap-1 px-2">
					...
					<div className="flex items-center gap-1">
						...
						<p className="text-lg font-semibold">{gym.location.text}</p>
					</div>
					<div className="flex flex-col items-center gap-1 ml-auto pr-5 mt-auto">

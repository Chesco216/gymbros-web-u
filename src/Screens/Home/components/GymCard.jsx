...
export const GymCard = ({ gym }) => {
	...
	return (
		<li className={`fade-in-section ${isVisible ? 'is-visible' : ''} flex bg-white rounded-xl shadow-lg hover:bg-gray-50 w-full cursor-pointer transition-all ease-in-out hover:scale-[1.020] h-80 lg:h-auto pb-10 border border-gray-300 duration-200`} ref={sectionRef}>
			<NavLink to={`/gyms/${gym.uid}`} className="flex flex-col w-full items-start gap-4">
				<img src={gym.main_image} alt="" className="w-full h-1/3 xl:h-2/3 object-cover rounded-t-xl" />
